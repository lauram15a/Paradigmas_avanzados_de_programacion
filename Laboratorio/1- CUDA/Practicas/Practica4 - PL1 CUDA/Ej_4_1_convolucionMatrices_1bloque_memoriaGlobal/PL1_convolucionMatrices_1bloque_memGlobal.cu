...
int main()
{
    ...
    dim3 dimBlock(width_RGB, width_RGB);

    // Invocamos al Kernell
    sumaMatrices << < 1, dimBlock >> > (r_, g_, b_, rgb_);
    ...
    // Invocamos al Kernell 
    matrizVolteada << <1, dimBlock_filtro >> > (filtro_, filtro_volt_);
    ...
    // Invocamos al Kernell
    convolucion << <1, dimBlock >> > (rgb_, filtro_volt_, resultado_);
